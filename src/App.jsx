...
function PlanetSignCard({ name, p, pill }) {
  ...
  // Use hardcoded outer planet range if available, otherwise use lookup table pill
  const outerRange = getOuterPlanetRange(name, p.sign);
  const displayPill = outerRange || pill;
  return (
    <div className="sky-card" onClick={() => setOpen(v => !v)} style={{ cursor:"pointer" }}>
      <div className="sky-card-top">
        ...
        <div style={{ display:"flex", alignItems:"center", gap:"0.4rem", flexShrink:0 }}>
          {displayPill && <span className="sky-card-pill">{displayPill}</span>}
          <span style={{ fontSize:"0.75rem", color:"#555" }}>{open ? "▲" : "▼"}</span>
        </div>
      </div>
      {open && (
...
// ─── OUTER PLANET DATE RANGES ─────────────────────────────────────────────────
// Hardcoded known ingress dates for slow-moving planets (accurate, won't change)
const OUTER_PLANET_RANGES = {
  "jupiter-gemini":      { start: "May 2024",  end: "Jun 2025"  },
  "jupiter-cancer":      { start: "Jun 2025",  end: "Jun 2026"  },
  "jupiter-leo":         { start: "Jun 2026",  end: "Jul 2027"  },
  "saturn-pisces":       { start: "Mar 2023",  end: "May 2025"  },
  "saturn-aries":        { start: "May 2025",  end: "Sep 2025"  },
  "saturn-pisces-2":     { start: "Sep 2025",  end: "Feb 2026"  },
  "saturn-aries-2":      { start: "Feb 2026",  end: "Apr 2028"  },
  "uranus-taurus":       { start: "May 2018",  end: "Jul 2025"  },
  "uranus-gemini":       { start: "Jul 2025",  end: "Apr 2033"  },
  "neptune-pisces":      { start: "Feb 2012",  end: "Mar 2025"  },
  "neptune-aries":       { start: "Mar 2025",  end: "Jan 2039"  },
  "pluto-capricorn":     { start: "Nov 2008",  end: "Jan 2024"  },
  "pluto-aquarius":      { start: "Jan 2024",  end: "Sep 2043"  },
};

function getOuterPlanetRange(planetName, signName) {
  // Try exact key first, then with suffix
  const key = `${planetName}-${signName}`;
  const entry = OUTER_PLANET_RANGES[key]
    || OUTER_PLANET_RANGES[key + "-2"]
    || Object.entries(OUTER_PLANET_RANGES).find(([k]) => k.startsWith(key))?.[1];
          ...
          <section className="sec" id="today">
            ...
            {(() => {
              ...
              const cards = planetOrder.map(name => {
                ...
                const pill = getOuterPlanetRange(name, p.sign) || dateRangePill(rangeKey, planetSignRanges);
          ...
          <section className="sec" id="month">
            <div className="sec-hdr">
              ...
              {(() => {
                ...
                const buildRow = (year, monthNum, d, dayData, detDay) => {
                  const date       = new Date(year, monthNum - 1, d);
                  const dateStr    = `${MONTH_NAMES_FULL[monthNum-1]} ${d}, ${year}`;
                  ...
                  return [dateStr, dayName, moonSign, moonPhase, illum,
                    esc(voc), esc(enters), esc(planetChanges.join("; ")), esc(aspectList.join("; "))
                  ].join(",");
                };

                const downloadCSV = (rows, filename) => {
                  const csv  = ["Date,Day,Moon Sign,Moon Phase,Illumination,Void of Course,Moon Enters,Planet Sign Changes,Aspects", ...rows].join("\n");
                ...
                const exportMonth = () => {
                  ...
                  for (let d = 1; d <= daysInCurrentMonth; d++) {
                    ...
                    rows.push(buildRow(currentYear, currentMonth, d, transitData?.[mk]?.[d], detailedData?.days?.[d]));
                  }
                  downloadCSV(rows, `keys-to-the-stars-${currentYear}-${String(currentMonth).padStart(2,"0")}.csv`);
                };

                const exportRest = () => {
                  const rows = [];
                  for (let mo = currentMonth; mo <= 12; mo++) {
                    const mk = `${currentYear}-${String(mo).padStart(2,"0")}`;
                    const daysInMo = new Date(currentYear, mo, 0).getDate();
                    for (let d = 1; d <= daysInMo; d++) {
                      rows.push(buildRow(currentYear, mo, d, transitData?.[mk]?.[d], null));
                    }
                  }
                  downloadCSV(rows, `keys-to-the-stars-${currentYear}-rest-of-year.csv`);
                };
